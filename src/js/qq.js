...
const qq = {
    ...
    playsquare: (playlistIdArr, callback) => {
        const playlistUrl = 'https://c.y.qq.com/qzone/fcg-bin/fcg_ucc_getcdinfo_byids_cp.fcg?type=1&format=json&outCharset=utf-8&disstid=';
        fetch(playlistUrl + playlistIdArr[2]).
    ...
    album: (playlistIdArr, callback) => {
        const playlistUrl = 'https://c.y.qq.com/v8/fcg-bin/fcg_v8_album_info_cp.fcg?albummid=';
        fetch(playlistUrl + playlistIdArr[2]).
            then(response => response.json()).
            then(function(r) {
                let playlist = [];
    ...
    singer: (playlistIdArr, callback) => {
        const playlistUrl = 'https://c.y.qq.com/v8/fcg-bin/fcg_v8_singer_track_cp.fcg?order=listen&num=1000&singermid=';
        fetch(playlistUrl + playlistIdArr[2]).
            then(response => response.json()).
            then(function(r) {
                let playlist = [];
                ...
                console.log('singer', playlist);
                callback(playlist);
            });
    },
    song: (playlistIdArr, callback) => {
        const playlistUrl = 'https://c.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?format=json&songmid=';
        fetch(playlistUrl + playlistIdArr[2]).
            then(response => response.json()).
            then(function(r) {
                let playlist = [];
    ...
    getSong: (song, callback) => {
        ...
        let vkey = '';
        let songUrl = 'http://dl.stream.qqmusic.qq.com/' +
            qq.songType.size24.t + mstr + qq.songType.size24.e;
        if (!localObj || now > localObj.e) {
            const keyUrl = 'http://c.y.qq.com/base/fcgi-bin/fcg_music_express_mobile3.fcg?';
            fetch(
                keyUrl + 'cid=' + 205361747 + '&songmid=' + mid + '&filename=' +
                'C400' + mstr + '.m4a' + '&guid=' + guid).
                then(response => response.json()).
                then(function(r) {
                    const expiration = now + r.data.expiration * 1000;
                    if (r.data.items.length < 1 || r.data.items[0].subcode !==
                        0) {
                        localStorage.setItem(mstr,
                            JSON.stringify({v: vkey, e: expiration, g: guid}));
                    ...
                    localStorage.setItem(mstr,
                        JSON.stringify({v: vkey, e: expiration, g: guid}));
                    songUrl = 'http://dl.stream.qqmusic.qq.com/' + type.t +
                        mstr + type.e + '?guid=' + guid + '&fromtag=64&vkey=' +
                        vkey;
            ...
            vkey = localObj.v;
            if (vkey) {
                songUrl = 'http://dl.stream.qqmusic.qq.com/' + type.t + mstr +
                    type.e + '?guid=' + guid + '&fromtag=64&vkey=' + vkey;
            } else {
                songUrl = 'http://dl.stream.qqmusic.qq.com/' +
                    qq.songType.size24.t + mstr + qq.songType.size24.e;
            }
            callback({src: songUrl});
        }
    ...
    getLyric: (song, callback) => {
        const keyUrl = 'https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg?songmid=';
        fetch(keyUrl + song.songmid).
